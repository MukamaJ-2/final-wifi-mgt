...
// Helper function to generate unique 4-digit number
const generateUnique4Digits = () => {
  return Math.floor(1000 + Math.random() * 9000).toString();
};

// Helper function to generate guest username in format "Guest@ucu.5986"
const generateGuestUsername = (adminEmail, baseUsername) => {
  const uniqueDigits = generateUnique4Digits();
  return `Guest@ucu.${uniqueDigits}`;
};

// Helper function to generate password in format "guest@ucu.ac.1546"
const generatePassword = () => {
  const uniqueDigits = generateUnique4Digits();
  return `guest@ucu.ac.${uniqueDigits}`;
};
...
export const createGuestUser = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      data: {
        user: newUser.data[0],
        plainPassword: generatedPassword, // Return plain password for display
